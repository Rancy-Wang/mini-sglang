...
class RadixPrefixCache(BasePrefixCache):
    ...
    def match_occurrence_retry_prefix(
        ...
        """Find the longest structurally compatible read-only occurrence source.

        Staged Retry keeps its historical greedy selection.  Here a candidate's
        advertised subtree depth is only an upper bound: compare actual records
        before selecting it, and stop at the first incompatible virtual marker.
        """

        del virtual_mask  # The Retry record comparator checks virtual rows itself.
        if target.ndim != 2 or target.shape[1] != 4:
            return exact_handle
        from minisgl.kernel.radix import fast_compare_retry_radix_records
        ...
        while frontier and -frontier[0][0] > best_len:
            ...
            matched = fast_compare_retry_radix_records(child._key, target[cursor:])
            if matched <= 0:
                continue
            end = cursor + matched
            if end > best_len:
