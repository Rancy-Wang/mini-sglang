...
def build_context_visibility_mask(
    ...
    *,
    ...
    if query_start + query_length > len(full_token_visible_until):
        raise ValueError("Context-mask query range exceeds token metadata.")
    if key_length > len(full_token_visible_until):
        raise ValueError("Context-mask key range exceeds token metadata.")
    ...
    if output is None:
        ...
        output = full_token_visible_until.new_empty(mask_length, dtype=torch.uint8)
...
def build_context_visibility_mask_packed(
    ...
    *,
    ...
    if output is None:
        output = full_token_visible_until.new_empty(packed_length, dtype=torch.uint8)
